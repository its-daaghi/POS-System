  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon="💰" iconBg="bg-emerald-900/50 text-emerald-400" label="Today Revenue"
          value={fmt(stats?.today?.revenue || 0)}
          sub={`${rc >= 0 ? '▲' : '▼'} ${Math.abs(rc).toFixed(1)}% vs yesterday`}
          subColor={rc >= 0 ? 'text-emerald-400' : 'text-red-400'} />
        <StatCard icon="🧾" iconBg="bg-blue-900/50 text-blue-400" label="Today Bills"
          value={String(stats?.today?.count || 0)}
          sub={`${bc >= 0 ? '▲' : '▼'} ${Math.abs(bc).toFixed(1)}% vs yesterday`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="card">
          <h3 className="font-semibold text-white mb-4">📊 Today vs Yesterday</h3>
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-gray-400">Yesterday Revenue</span>
        ...
        <div className="card">
          ...
          ) : <p className="text-gray-500 text-sm text-center py-8">No sales today</p>}
        </div>
